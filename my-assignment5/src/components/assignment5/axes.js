...
function YAxis(props) {
    const { yScale, height, offsetX, data} = props;

    return (
        <g transform={`translate(${offsetX}, 0)`}>
            {/* Draw the y-axis line */}
            <line x1={-130} y1={0} x2={-130} y2={height} stroke="black" />
            ...
            {yScale.domain().map(tickValue => 
                ...
                <g key={tickValue} transform={`translate(${0}, ${yScale(tickValue) })`}>
                    {/* Tick lines */}
                    <line x1={-135} x2={-130} y1={yScale.bandwidth()/2} y2={yScale.bandwidth()/2} stroke="black" />
                    {/* Tick text */}
                    <text style={{ textAnchor: "start", fontSize: "10px" }} x={-offsetX - 120} y={yScale.bandwidth()/2}>
                        {tickValue}
                    </text>
                </g>
            )}
